...
{
//   dasom_joint_states_pub_ = node_handle_.advertise<sensor_msgs::JointState>(robot_name_ + "/goal_dynamixel_position", 10);
    testerpub = node_handle_.advertise<geometry_msgs::Twist>("/tester", 10);
    forcepub = node_handle_.advertise<geometry_msgs::Twist>("/estimated_force",10);
}

void Force::initSubscriber()
{
//   goal_joint_states_sub_ = node_handle_.subscribe(robot_name_ + "/position_Force", 10, &Force::goaljointCallback, this);
    effortsub = node_handle_.subscribe("/joint_states", 1000, &Force::EffortToTorque,this);  //각도(변수 다름), 각속도(변수 다름), 각가속도, LPF 4개를 받아오는 거
    // xysub = node_handle_.subscribe("/gripper/kinematics_pose", 1000, &Force::XYCallback,this);
}

void Force::EffortToTorque(const sensor_msgs::JointState::ConstPtr& msg)
{
