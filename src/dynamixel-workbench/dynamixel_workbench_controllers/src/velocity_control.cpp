     ...
     //dxl_cnt_(2)
{
  ...
  uint32_t velocity_p_gain   = node_handle_.param<int>("velocity_p_gain", 100);
  uint32_t velocity_i_gain   = node_handle_.param<int>("velocity_i_gain", 1920);
...
void VelocityControl::initPublisher()
{
  ...
  dasom_joint_states_pub_ = node_handle_.advertise<dynamixel_workbench_msgs::DasomDynamixel>("/dasom/joint_states", 10);
}
...
void VelocityControl::controlLoop()
{
  ...
  for (int index = 0; index < dxl_cnt_; index++)
  {
    ...
    dynamixel_.position.push_back(dxl_wb_->convertValue2Radian(dxl_id_[index], (dxl_wb_->itemRead(dxl_id_[index], "Present_Position"))));
    dynamixel_.velocity.push_back(dxl_wb_->convertValue2Velocity(dxl_id_[index], (dxl_wb_->itemRead(dxl_id_[index], "Present_Velocity"))));
    dynamixel_.effort.push_back(dxl_wb_->convertValue2Torque(dxl_id_[index], (dxl_wb_->itemRead(dxl_id_[index], "Present_Current"))));
  
    position_for_FK[index] = dxl_wb_->convertValue2Radian(dxl_id_[index], (dxl_wb_->itemRead(dxl_id_[index], "Present_Position")));
  }
  ...
  // ROS_INFO("syncRead : %lf", raw_velocity[0]);
  // ROS_WARN("syncRead : %lf", raw_velocity[1]);
  // ROS_FATAL("syncRead : %lf", raw_velocity[2]);
  // ROS_INFO("syncRead : %lf", raw_velocity[3]);
  // ROS_WARN("syncRead : %lf", raw_velocity[4]);
  // ROS_FATAL("syncRead : %lf", raw_velocity[5]);
  ...
  //   dynamixel_.velocity.push_back(dxl_wb_->convertValue2Velocity(dxl_id_[index],raw_velocity[index]));
  //   dynamixel_.effort.push_back(dxl_wb_->convertValue2Torque(dxl_id_[index], present_effort[index]));
  // }
...
void VelocityControl::commandVelocityCallback(const geometry_msgs::Twist::ConstPtr &msg)
{
  ...
  // float wheel_velocity[dxl_cnt_] = {0.0, 0.0, };
  // int32_t dynamixel_velocity[dxl_cnt_] = {0, 0, };
...
void VelocityControl::ForwardKinematics()
{
  double Link1 = 0.10375;
	double Link2 = 0.108;
  double alpha = position_for_FK[0]; // 값 잘 받아오고 있음
  double beta = position_for_FK[1]; // 값 잘 받아오고 있음

  // ROS_INFO("joint1 = %lf", alpha);
  // ROS_INFO("joint2 = %lf", beta);
