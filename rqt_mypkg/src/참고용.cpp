...
//  ping_client = n.serviceClient<std_srvs::Empty>("/ping_tester");
  ...
  subscriber_ = n.subscribe("/dasom/camera_image", 1, &MyPlugin::callbackImage, this);




  //QObject::connect(ui_.image_frame, SIGNAL(mouseLeft(int, int)), this, SLOT(onMouseLeft(int, int)));
...
void MyPlugin::callback_set(const ros::TimerEvent&) 
{
  if(isCallback)
  {
    ...
    Eigen::Vector3d FK;
    FK <<   Link2 * cos(q1 + q2) + Link1 * cos(q1) + Link3 * cos(q1 + q2 + q3),
            Link2 * sin(q1 + q2) + Link1 * sin(q1) + Link3 * sin(q1 + q2 + q3),
            q1 + q2 + q3;
      
      ui_.txt_FK_x->setText((QString::number(FK[0])));
      ui_.txt_FK_y->setText((QString::number(FK[1])));
      ui_.txt_FK_z->setText((QString::number(FK[2])));
...
//0807) 여기 부분에다가 subscribe 한 내용을 이미지로 띄우는 코드를 작성하면 되겠지 싶습니다.
// 내 추측으로는 ui_.image_frame->어쩌고저쩌고     와 같은 형식으로 작성하는 것 같은데,
// 혹시 orocuor 프로젝트 진행할 때 영상정보 띄운 방식을 참고하면 수월할 것 같습다.
// rqt_image_View .cpp를 계속 뜯어봐야되나... 싶기도 하고...
