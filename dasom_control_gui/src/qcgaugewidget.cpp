...
QRectF QcItem::adjustRect(float percentage)
{
    ...
    QRectF tmpRect = mRect.adjusted(offset-35,offset-45,-offset,-offset-10);
    return tmpRect; // HERE
}
...
void QcLabelItem::draw(QPainter *painter)
{
    ...
    QFont font("Ubuntu", r/10.0, QFont::Light);
...
void QcDegreesItem::draw(QPainter *painter)
{
    ...
    // tmpRect.setRect(tmpRect.center().x() - 35, tmpRect.center().y() - 50, tmpRect.width(), tmpRect.height());
...
void QcValuesItem::draw(QPainter*painter)
{
    ...
    QFont font("Ubuntu",0, QFont::Light);
    ...
    for(float val = mMinValue;val<=mMaxValue;val+=mStep){
        ...
        QPainterPath path;
        path.moveTo(pt);
        path.lineTo(tmpRect.center()-QPointF(50,80));
